            ...
            return (
              <div
                ...
                className={`relative rounded-[3.5rem] overflow-hidden transition-all duration-700 bg-white cursor-pointer border border-slate-200 ${
                  isHovered && !isExpanded ? 'md:flex-[1.4] shadow-2xl border-blue-600' : 'md:flex-1 shadow-sm'
                }`}
              >
                {/* Background State */}
                <div className="absolute inset-0 bg-slate-900">
                  <div className={`absolute inset-0 bg-slate-50 transition-opacity duration-700 ${isHovered && !isExpanded ? 'opacity-10' : 'opacity-100'}`}>
                  </div>
                </div>

                {/* Default View */}
                <div className={`absolute inset-0 p-14 flex flex-col justify-end transition-all duration-500 ${
                  (isExpanded || (isHovered && !isExpanded)) ? 'md:opacity-0 md:translate-y-10' : 'opacity-100'
                ...
                <div className={`absolute inset-0 p-14 flex flex-col justify-end bg-blue-600 transition-all duration-700 transform ${
                  isExpanded || (isHovered && !isExpanded) ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'
                }`}>
                  ...
                  {isExpanded && (
                    <button 
                      ...
                      className="absolute top-6 right-6 w-10 h-10 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white md:hidden"
                    >
                      <X className="w-5 h-5" />
                    </button>
                  )}
