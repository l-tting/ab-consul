          ...
          {team.map((member, i) => (
            <div
              ...
              className={`relative rounded-[3.5rem] overflow-hidden transition-all bg-white duration-700 cursor-pointer border border-slate-200
                md:flex-1
                ${hoveredIndex === i ? 'md:flex-[1.4] shadow-2xl border-blue-600' : 'shadow-sm'}
                w-full mb-8 md:mb-0
              `}
            >
              {/* Background State */}
              ...
              {/* Default View */}
              <div className={`absolute inset-0 p-14 flex flex-col justify-end transition-all duration-500 ${hoveredIndex === i ? 'opacity-0 translate-y-10 md:opacity-0 md:translate-y-10' : 'opacity-100 translate-y-0'}`}>
