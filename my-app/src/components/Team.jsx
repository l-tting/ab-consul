...
export default function TeamSection() {
  ...
  const team = [
    {
      name: "Andrew Letting",
      title: "Strategic Operator & Growth Architect",
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-center items-stretch ">
          {team.map((member, i) => (
            <div
              ...
              className={`relative min-h-[550px] rounded-[3rem] overflow-hidden transition-all bg-white duration-700 cursor-pointer border border-slate-300 shadow-sm ${hoveredIndex === i ? 'md:flex-[1.4] shadow-2xl' : 'md:flex-1'
                }`}
            >
              {/* Background State */}
              ...
              <div className={`absolute inset-0 p-12 flex flex-col justify-end transition-all duration-500 ${hoveredIndex === i ? 'opacity-0 translate-y-10' : 'opacity-100'}`}>
                <div className="space-y-1">
                  <p className="text-[13px] font-bold text-blue-600 uppercase tracking-widest">{member.title}</p>
              ...
              <div className={`absolute inset-0 p-12 flex flex-col justify-end bg-blue-600 transition-all duration-500 transform ${hoveredIndex === i ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'
                }`}>
